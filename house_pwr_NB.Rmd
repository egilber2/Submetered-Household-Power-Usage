...
In this section, we'll go over  data munging steps
...
```
Now that the DateTime feature has been converted to the proper format, we can use the range() function to understand the timeframe that the data set covers.
...
####**4.1.6 Hour of the Day Time Period**
...
house_pwr_tidy %>%
  ...
  theme(panel.border=element_rect(colour='black', fill=NA))
```
###4.2 Compare High Energy Consumption for Day of Week (Summer & Winter) {.tabset .tabset-pills}
...
###**5.1 Quarterly** 
Subset data by quarterly time frame.
```{r}
...
```
###**5.2 Monthly** 
Subset data by monthly time frame.
